...
import CanvasChakraAura from '@/lib/canvas/CanvasChakraAura';
import { CanvasTextureLayer } from './CanvasTextureLayer';

export function ChakraAuraDisplayLayer({
  display,
  order,
  frameData,
  sceneOpacity,
  ...
  const drawFrame = React.useCallback(({ canvas, properties, frameData }) => {
    ...
    if (!canvasRef.current) {
      canvasRef.current = new CanvasChakraAura(properties, canvas);
    }
