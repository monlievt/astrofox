...
import CanvasBinauralResonance from '@/lib/canvas/CanvasBinauralResonance';
import { CanvasTextureLayer } from './CanvasTextureLayer';

export function BinauralResonanceDisplayLayer({
  display,
  order,
  frameData,
  sceneOpacity,
  ...
  const drawFrame = React.useCallback(({ canvas, properties, frameData }) => {
    ...
    if (!canvasRef.current) {
      canvasRef.current = new CanvasBinauralResonance(properties, canvas);
    }
