...
import CanvasQuantumSiriSphere from '@/lib/canvas/CanvasQuantumSiriSphere';
import { CanvasTextureLayer } from './CanvasTextureLayer';

export function QuantumSiriSphereDisplayLayer({
  display,
  order,
  frameData,
  sceneOpacity,
  ...
  const drawFrame = React.useCallback(({ canvas, properties, frameData }) => {
    ...
    if (!canvasRef.current) {
      canvasRef.current = new CanvasQuantumSiriSphere(properties, canvas);
    }
