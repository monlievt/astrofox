...
import CanvasMonstercat from '@/lib/canvas/CanvasMonstercat';
import { CanvasTextureLayer } from './CanvasTextureLayer';

export function MonstercatDisplayLayer({
  display,
  order,
  frameData,
  sceneOpacity,
  ...
  const monstercatRef = React.useRef(null);

  const drawFrame = React.useCallback(({ canvas, properties, frameData }) => {
    if (!monstercatRef.current) {
      monstercatRef.current = new CanvasMonstercat(properties, canvas);
    }

    monstercatRef.current.update(properties);
    ...
    monstercatRef.current.render(floatFft);
