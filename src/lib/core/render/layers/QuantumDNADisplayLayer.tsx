...
import CanvasQuantumDNA from '@/lib/canvas/CanvasQuantumDNA';
import { CanvasTextureLayer } from './CanvasTextureLayer';

export function QuantumDNADisplayLayer({
  display,
  order,
  frameData,
  sceneOpacity,
  ...
  const drawFrame = React.useCallback(({ canvas, properties, frameData }) => {
    ...
    if (!canvasRef.current) {
      canvasRef.current = new CanvasQuantumDNA(properties, canvas);
    }
