...
import CanvasNeuroFlow from '@/lib/canvas/CanvasNeuroFlow';
import { CanvasTextureLayer } from './CanvasTextureLayer';

export function NeuroFlowFieldDisplayLayer({
  display,
  order,
  frameData,
  sceneOpacity,
  ...
  const drawFrame = React.useCallback(({ canvas, properties, frameData }) => {
    ...
    if (!canvasRef.current) {
      canvasRef.current = new CanvasNeuroFlow(properties, canvas);
    }
