...
import CanvasMorphingSphere from '@/lib/canvas/CanvasMorphingSphere';
import { CanvasTextureLayer } from './CanvasTextureLayer';

export function MorphingSphereDisplayLayer({
  display,
  order,
  frameData,
  sceneOpacity,
  ...
  const drawFrame = React.useCallback(({ canvas, properties, frameData }) => {
    if (!sphereRef.current) {
      sphereRef.current = new CanvasMorphingSphere(properties, canvas);
    }
