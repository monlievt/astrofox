...
import CanvasWaveHorizon from '@/lib/canvas/CanvasWaveHorizon';
import { CanvasTextureLayer } from './CanvasTextureLayer';

export function WaveHorizonDisplayLayer({
  display,
  order,
  frameData,
  sceneOpacity,
  ...
  const drawFrame = React.useCallback(({ canvas, properties, frameData }) => {
    ...
    if (!canvasRef.current) {
      canvasRef.current = new CanvasWaveHorizon(properties, canvas);
    }
